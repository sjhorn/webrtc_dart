/**
 * Automated TWCC (Transport-Wide Congestion Control) Browser Test using Playwright
 *
 * Tests WebRTC TWCC negotiation:
 * - Browser sends VP8 video to Dart with TWCC enabled
 * - Verifies transport-cc RTCP feedback is negotiated
 * - Verifies TWCC header extensions (abs-send-time, transport-wide-cc)
 *
 * Usage:
 *   # First, start the Dart server in another terminal:
 *   dart run interop/automated/twcc_server.dart
 *
 *   # Then run browser tests:
 *   node interop/automated/twcc_test.mjs [chrome|firefox|webkit|all]
 */

import { chromium, firefox, webkit } from 'playwright';

const SERVER_URL = 'http://localhost:8779';
const TEST_TIMEOUT = 60000;

async function runBrowserTest(browserType, browserName) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing TWCC: ${browserName}`);
  ...
  try {
    ...
    console.log(`[${browserName}] Running TWCC test (5 seconds)...`);
    ...
    console.log(`  TWCC Negotiated: ${result.twccNegotiated}`);
    ...
    if (result.twccSdpInfo) {
      console.log(`  TWCC SDP Info:\n${result.twccSdpInfo}`);
    }
    if (result.error) {
...
async function main() {
  ...
  console.log('WebRTC TWCC (Transport-Wide Congestion Control) Browser Test');
  console.log('=============================================================');
  ...
  try {
    ...
    console.error('\nError: TWCC server is not running!');
    console.error('Start it with: dart run interop/automated/twcc_server.dart');
    process.exit(1);
  }
