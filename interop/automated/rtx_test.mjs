/**
 * Automated RTX (Retransmission) Browser Test using Playwright
 *
 * Tests WebRTC RTX codec negotiation:
 * - Browser sends VP8 video to Dart with RTX enabled
 * - Verifies RTX codec is negotiated in SDP
 * - Verifies RTX attributes (rtpmap, fmtp apt=, ssrc-group FID)
 *
 * Usage:
 *   # First, start the Dart server in another terminal:
 *   dart run interop/automated/rtx_server.dart
 *
 *   # Then run browser tests:
 *   node interop/automated/rtx_test.mjs [chrome|firefox|webkit|all]
 */
...
async function runBrowserTest(browserType, browserName) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing RTX: ${browserName}`);
  ...
  try {
    ...
    console.log(`[${browserName}] Running RTX test (5 seconds)...`);
    ...
    console.log(`  RTX Negotiated: ${result.rtxNegotiated}`);
    ...
    if (result.rtxSdpInfo) {
      console.log(`  RTX SDP Info:\n${result.rtxSdpInfo}`);
    }
    if (result.error) {
...
async function main() {
  ...
  console.log('WebRTC RTX (Retransmission) Browser Test');
  console.log('========================================');
  ...
  try {
    ...
    console.error('\nError: RTX server is not running!');
    console.error('Start it with: dart run interop/automated/rtx_server.dart');
    process.exit(1);
  }
