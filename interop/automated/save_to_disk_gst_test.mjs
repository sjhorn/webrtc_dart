/**
 * Automated Save to Disk GStreamer Browser Test using Playwright
 *
 * Tests WebRTC video recording via GStreamer pipeline:
 * - Browser sends VP8 camera video to Dart
 * - Dart forwards raw RTP packets via UDP to GStreamer
 * - GStreamer handles depacketization and muxes to WebM file
 * - Verifies file was created with non-zero size
 *
 * Usage:
 *   # First, start the Dart server in another terminal:
 *   dart run interop/automated/save_to_disk_gst_server.dart
 *
 *   # Then run browser tests:
 *   node interop/automated/save_to_disk_gst_test.mjs [chrome|firefox|webkit|all]
 *
 * Requirements: GStreamer 1.x with plugins: udpsrc, rtpvp8depay, webmmux
 */
...
async function runBrowserTest(browserType, browserName) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing Save to Disk GStreamer: ${browserName}`);
  ...
  try {
    ...
    console.log(`[${browserName}] Running GStreamer test (5 seconds)...`);
    ...
    console.log(`  Packets Forwarded: ${result.packetsForwarded || 0}`);
...
async function main() {
  ...
  console.log('WebRTC Save to Disk GStreamer Browser Test');
  console.log('==========================================');
  ...
  try {
    ...
    console.error('\nError: Save to disk GStreamer server is not running!');
    console.error('Start it with: dart run interop/automated/save_to_disk_gst_server.dart');
    process.exit(1);
  }
  ...
  console.log('\n' + '='.repeat(60));
  console.log('SAVE TO DISK GSTREAMER TEST SUMMARY');
  console.log('='.repeat(60));
  ...
  for (const result of results) {
    ...
    if (result.success) {
      console.log(`       Packets: ${result.packetsReceived || 0} recv, ${result.packetsForwarded || 0} fwd`);
