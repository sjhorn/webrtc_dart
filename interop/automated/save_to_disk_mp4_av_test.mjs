/**
 * Automated Save to Disk MP4 A/V Browser Test using Playwright
 *
 * Tests WebRTC audio+video recording to fragmented MP4:
 * - Browser sends H.264 camera video + Opus microphone audio to Dart server
 * - Dart depacketizes H.264 RTP and Opus, writes to fMP4 container
 * - Verifies MP4 file is created with valid size
 *
 * Usage:
 *   # First, start the Dart server in another terminal:
 *   dart run interop/automated/save_to_disk_mp4_av_server.dart
 *
 *   # Then run browser tests:
 *   node interop/automated/save_to_disk_mp4_av_test.mjs [chrome|firefox|webkit|all]
 */
...
async function runBrowserTest(browserType, browserName) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing Save to Disk MP4 A/V: ${browserName}`);
  ...
  try {
    ...
    console.log(`[${browserName}] Running save to disk MP4 A/V test...`);
    ...
    console.log(`  Video Frames: ${result.videoFramesWritten || 0}`);
    console.log(`  Audio Frames: ${result.audioFramesWritten || 0}`);
...
async function main() {
  ...
  console.log('WebRTC Save to Disk MP4 A/V Browser Test');
  console.log('========================================');
  ...
  try {
    ...
    console.error('\nError: Save to Disk MP4 A/V server is not running!');
    console.error('Start it with: dart run interop/automated/save_to_disk_mp4_av_server.dart');
    process.exit(1);
  }
  ...
  console.log('\n' + '='.repeat(60));
  console.log('SAVE TO DISK MP4 A/V TEST SUMMARY');
  console.log('='.repeat(60));
  ...
  for (const result of results) {
    ...
    if (result.success) {
      console.log(`       Video: ${result.videoPacketsReceived} pkts, ${result.videoFramesWritten} frames`);
      console.log(`       Audio: ${result.audioPacketsReceived} pkts, ${result.audioFramesWritten} frames`);
