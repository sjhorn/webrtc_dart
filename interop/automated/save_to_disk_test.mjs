...
async function runBrowserTest(browserType, browserName) {
  ...
  try {
    ...
    // Wait for test to complete (recording takes ~7 seconds)
...
async function main() {
  ...
  try {
    ...
    console.error('\nError: Save to disk server is not running!');
    console.error('Start it with: dart run interop/automated/save_to_disk_server.dart');
    process.exit(1);
  }
