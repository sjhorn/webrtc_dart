// TWCC (Transport-Wide Congestion Control) Browser Test Server
//
// This server demonstrates TWCC functionality:
// 1. Configures VP8 codec with transport-cc RTCP feedback
// 2. Negotiates TWCC header extensions (abs-send-time, transport-wide-cc)
// 3. Verifies TWCC is enabled in SDP
//
// TWCC enables bandwidth estimation by tracking packet arrival times
// across the entire transport, not just individual streams.
//
...
class TwccServer {
  ...
  bool _twccNegotiated = false;
  String? _twccSdpInfo;

  TwccServer({int recordingDurationSeconds = 5})
      : _recordingDurationSeconds = recordingDurationSeconds;

  Future<void> start({int port = 8779}) async {
    _server = await HttpServer.bind(InternetAddress.anyIPv4, port);
    print('[TWCC] Started on http://localhost:$port');
  ...
  Future<void> _handleRequest(HttpRequest request) async {
    ...
    print('[TWCC] ${request.method} $path');
  ...
  Future<void> _handleStart(HttpRequest request) async {
    ...
    print('[TWCC] Starting test for: $_currentBrowser');
    ...
    _twccNegotiated = false;
    _twccSdpInfo = null;
    _outputPath =
        './recording-twcc-${DateTime.now().millisecondsSinceEpoch}.webm';
        ...
        video: [
          ...
          createVp8Codec(
            ...
            rtcpFeedback: [
              ...
              RtcpFeedbackTypes.transportCC, // TWCC feedback
    ...
    print('[TWCC] PeerConnection created with VP8 + TWCC');

    _pc!.onConnectionStateChange.listen((state) {
      print('[TWCC] Connection state: $state');
    ...
    _pc!.onIceConnectionStateChange.listen((state) {
      print('[TWCC] ICE state: $state');
    });

    _pc!.onIceCandidate.listen((candidate) {
      print(
          '[TWCC] Local ICE candidate: ${candidate.type} ${candidate.host}:${candidate.port}');
    ...
    print('[TWCC] Added video transceiver (recvonly)');

    _pc!.onTrack.listen((transceiver) async {
      print('[TWCC] Received track: ${transceiver.kind}');
        ...
        onRtp: (handler) {
          track.onReceiveRtp.listen((rtp) {
            ...
            if (_rtpPacketsReceived % 100 == 0) {
              print('[TWCC] Received $_rtpPacketsReceived RTP packets');
            }
          });
        },
      );
      ...
      print('[TWCC] Recording started to: $_outputPath');

      _recordingTimer =
          Timer(Duration(seconds: _recordingDurationSeconds), () async {
        print('[TWCC] Recording duration reached, stopping...');
  ...
  Future<void> _stopRecording() async {
    ...
    print('[TWCC] Stopping recorder...');
    await _recorder!.stop();
    _recorder = null;
    print('[TWCC] Recording stopped');

    if (_outputPath != null) {
      final file = File(_outputPath!);
      if (await file.exists()) {
        final size = await file.length();
        print('[TWCC] Output file: $_outputPath ($size bytes)');
      } else {
        print('[TWCC] WARNING: Output file not created!');
      }
    }
  }
  ...
  Future<void> _handleOffer(HttpRequest request) async {
    ...
    print('[TWCC] Created offer, local description set');

    // Check TWCC in SDP
    final sdp = offer.sdp;
    _twccSdpInfo = _extractTwccInfo(sdp);
    if (_twccSdpInfo != null) {
      print('[TWCC] TWCC features in offer:');
      print(_twccSdpInfo);
    }
    ...
    print('[TWCC] Sent offer to browser');
  }

  String? _extractTwccInfo(String sdp) {
    final lines = <String>[];
  ...
  Future<void> _handleAnswer(HttpRequest request) async {
    ...
    print('[TWCC] Received answer from browser');

    // Check TWCC in answer
    final answerTwccInfo = _extractTwccInfo(answer.sdp);
    if (answerTwccInfo != null) {
      print('[TWCC] TWCC in answer:');
      print(answerTwccInfo);
      _twccNegotiated = true;
    }

    await _pc!.setRemoteDescription(answer);
    print('[TWCC] Remote description set');
  ...
  Future<void> _handleCandidate(HttpRequest request) async {
    ...
    if (candidateStr.isEmpty || candidateStr.trim().isEmpty) {
      print('[TWCC] Skipping empty ICE candidate');
      request.response.headers.contentType = ContentType.json;
      request.response.write(jsonEncode({'status': 'ok'}));
      return;
    }
    ...
    try {
      ...
      print('[TWCC] Added ICE candidate: ${candidate.type}');
    } catch (e) {
      print('[TWCC] Failed to add candidate: $e');
    }
  ...
  Future<void> _handleStatus(HttpRequest request) async {
    ...
    request.response.write(jsonEncode({
      ...
      'twccNegotiated': _twccNegotiated,
      'twccSdpInfo': _twccSdpInfo,
...
<html>
<head>
    <title>WebRTC TWCC (Transport-Wide Congestion Control) Test</title>
    <style>
        ...
        .twcc-badge { background: #08a; color: #fff; padding: 2px 6px; border-radius: 4px; margin-left: 5px; font-size: 0.8em; }
        .stats { display: flex; gap: 20px; margin: 10px 0; }
        .stat { background: #333; padding: 10px; border-radius: 4px; }
        .stat-value { font-size: 1.5em; color: #8f8; }
        .twcc-info { background: #222; padding: 10px; margin: 10px 0; border-left: 3px solid #08a; font-size: 0.9em; }
    </style>
</head>
<body>
    <h1>TWCC Test
        <span class="codec-badge">VP8</span>
        <span class="twcc-badge">TWCC</span>
    </h1>
    <p>Tests Transport-Wide Congestion Control negotiation.</p>
    <div id="status">Status: Waiting to start...</div>
    <video id="preview" autoplay muted playsinline></video>
    <div class="stats">
        <div class="stat">Packets: <span id="packets" class="stat-value">0</span></div>
        <div class="stat">TWCC: <span id="twcc" class="stat-value">?</span></div>
    </div>
    <div id="twcc-info" class="twcc-info" style="display: none;"></div>
    <div id="log"></div>
    ...
    <script>
        ...
        async function runTest() {
            try {
                ...
                setStatus('Starting TWCC test for ' + browser);
                ...
                log('Server peer started (VP8 + TWCC)');
                ...
                // Check for TWCC in SDP
                if (offer.sdp.includes('transport-cc')) {
                    log('transport-cc RTCP feedback found in offer', 'success');
                }
                if (offer.sdp.includes('transport-wide-cc')) {
                    log('transport-wide-cc extension found in offer', 'success');
                }
                if (offer.sdp.includes('abs-send-time')) {
                    log('abs-send-time extension found in offer', 'success');
                }
                ...
                // Check TWCC in answer
                if (answer.sdp.includes('transport-cc')) {
                    log('transport-cc in answer', 'success');
                }
                ...
                setStatus('Sending video with TWCC (5 seconds)...');
                log('Sending video...');
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    document.getElementById('twcc').textContent = status.twccNegotiated ? 'YES' : 'NO';
                    document.getElementById('twcc').style.color = status.twccNegotiated ? '#8f8' : '#f88';

                    if (status.twccSdpInfo) {
                        const twccDiv = document.getElementById('twcc-info');
                        twccDiv.style.display = 'block';
                        twccDiv.innerHTML = '<strong>TWCC SDP Info:</strong><br><pre>' +
                            status.twccSdpInfo.replace(/\\n/g, '<br>') + '</pre>';
                    }

                    log('Packets: ' + (status.rtpPacketsReceived || 0) +
                        ', TWCC: ' + (status.twccNegotiated ? 'YES' : 'NO') +
                ...
                if (result.success) {
                    log('TEST PASSED! TWCC negotiated, ' + result.fileSize + ' bytes recorded', 'success');
                    setStatus('TEST PASSED - TWCC working');
                } else {
                    if (!result.twccNegotiated) {
                        log('TEST FAILED - TWCC not negotiated', 'error');
                    } else {
...
void main() async {
  final server = TwccServer(recordingDurationSeconds: 5);
  await server.start(port: 8779);
}
