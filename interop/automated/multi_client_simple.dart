...
class SimplifiedMultiClientServer {
  ...
  Future<void> start({int port = 8784}) async {
    _server = await HttpServer.bind(InternetAddress.anyIPv4, port);
    print('[Simple] Started on http://localhost:$port');
  ...
  Future<void> _handleRequest(HttpRequest request) async {
    ...
    print('[Simple] ${request.method} $path');
  ...
  Future<void> _handleStart(HttpRequest request) async {
    ...
    print('[Simple] Starting test for: $_currentBrowser');
  ...
  // KEY DIFFERENCE: This creates PC in /connect instead of /start
  Future<void> _handleConnect(HttpRequest request) async {
    ...
    print('[Simple] PeerConnection created');

    _pc!.onConnectionStateChange.listen((state) {
      print('[Simple] Connection state: $state');
    });

    _pc!.onIceConnectionStateChange.listen((state) {
      print('[Simple] ICE state: $state');
    });

    _pc!.onIceCandidate.listen((candidate) {
      print('[Simple] ICE candidate: ${candidate.type}');
  ...
  Future<void> _handleOffer(HttpRequest request) async {
    ...
    print('[Simple] Created offer');

    final hasSctp = offer.sdp.contains('m=application');
    print('[Simple] SDP has SCTP: $hasSctp');
  ...
  Future<void> _handleAnswer(HttpRequest request) async {
    ...
    print('[Simple] Received answer from browser');
    await _pc!.setRemoteDescription(answer);
    print('[Simple] Remote description set');
  ...
  Future<void> _handleCandidate(HttpRequest request) async {
    ...
    try {
      ...
      print('[Simple] Failed to add candidate: $e');
    }
...
<html>
<head>
    <title>Simplified Multi-Client Test</title>
    <style>
...
<body>
    <h1>Simplified Multi-Client Test</h1>
    <p>Tests if /connect vs /start is the issue.</p>
    <div id="log"></div>
    ...
    <script>
        ...
        async function runTest() {
            try {
                ...
                // KEY DIFFERENCE: Call /connect to create PC (instead of /start)
                ...
                // Get offer (creates DC on server)
                ...
                if (result.success) {
                    ...
                    log('TEST FAILED: ' + JSON.stringify(result), 'error');
                }
...
void main() async {
  ...
  final server = SimplifiedMultiClientServer();
  await server.start(port: 8784);
}
