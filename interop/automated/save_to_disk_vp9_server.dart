// Save to Disk VP9 Server for Automated Browser Testing
//
// This server:
// 1. Serves static HTML for browser to send camera video (VP9)
// 2. Creates a PeerConnection configured for VP9 codec
...
// Output: WebM file with VP9 video
//
// Note: VP9 is NOT supported by Safari. Only Chrome works.
...
class SaveToDiskVp9Server {
  ...
  SaveToDiskVp9Server({int recordingDurationSeconds = 5})
      : _recordingDurationSeconds = recordingDurationSeconds;

  Future<void> start({int port = 8771}) async {
    _server = await HttpServer.bind(InternetAddress.anyIPv4, port);
    print('[SaveToDisk-VP9] Started on http://localhost:$port');
  ...
  Future<void> _handleRequest(HttpRequest request) async {
    ...
    print('[SaveToDisk-VP9] ${request.method} $path');
  ...
  Future<void> _handleStart(HttpRequest request) async {
    ...
    print('[SaveToDisk-VP9] Starting VP9 test for: $_currentBrowser');
    ...
    print('[SaveToDisk-VP9] PeerConnection created with VP9 codec');

    _pc!.onConnectionStateChange.listen((state) {
      print('[SaveToDisk-VP9] Connection state: $state');
    ...
    _pc!.onIceConnectionStateChange.listen((state) {
      print('[SaveToDisk-VP9] ICE state: $state');
    });

    _pc!.onIceCandidate.listen((candidate) {
      print(
          '[SaveToDisk-VP9] Local ICE candidate: ${candidate.type} ${candidate.host}:${candidate.port}');
    ...
    print('[SaveToDisk-VP9] Added video transceiver (recvonly)');

    _pc!.onTrack.listen((transceiver) async {
      print('[SaveToDisk-VP9] Received track: ${transceiver.kind}');
        ...
        onRtp: (handler) {
          track.onReceiveRtp.listen((rtp) {
            ...
            if (_rtpPacketsReceived % 100 == 0) {
              print(
                  '[SaveToDisk-VP9] Received $_rtpPacketsReceived RTP packets');
            }
          });
        },
      );
      ...
      print('[SaveToDisk-VP9] Recording started to: $_outputPath');

      _recordingTimer =
          Timer(Duration(seconds: _recordingDurationSeconds), () async {
        print('[SaveToDisk-VP9] Recording duration reached, stopping...');
  ...
  Future<void> _stopRecording() async {
    ...
    print('[SaveToDisk-VP9] Stopping recorder...');
    await _recorder!.stop();
    _recorder = null;
    print('[SaveToDisk-VP9] Recording stopped');

    if (_outputPath != null) {
      final file = File(_outputPath!);
      if (await file.exists()) {
        final size = await file.length();
        print('[SaveToDisk-VP9] Output file: $_outputPath ($size bytes)');
      } else {
        print('[SaveToDisk-VP9] WARNING: Output file not created!');
      }
    }
  }
  ...
  Future<void> _handleOffer(HttpRequest request) async {
    ...
    print('[SaveToDisk-VP9] Created offer, local description set');
    ...
    print('[SaveToDisk-VP9] Sent offer to browser');
  }
  ...
  Future<void> _handleAnswer(HttpRequest request) async {
    ...
    print('[SaveToDisk-VP9] Received answer from browser');
    await _pc!.setRemoteDescription(answer);
    print('[SaveToDisk-VP9] Remote description set');
  ...
  Future<void> _handleCandidate(HttpRequest request) async {
    ...
    if (candidateStr.isEmpty || candidateStr.trim().isEmpty) {
      print('[SaveToDisk-VP9] Skipping empty ICE candidate');
      request.response.headers.contentType = ContentType.json;
      request.response.write(jsonEncode({'status': 'ok'}));
      return;
    }
    ...
    try {
      ...
      print('[SaveToDisk-VP9] Failed to add candidate: $e');
    }
...
<html>
<head>
    <title>WebRTC Save to Disk VP9 Test</title>
    <style>
...
<body>
    <h1>WebRTC Save to Disk Test <span class="codec-badge">VP9</span></h1>
    <p>Browser sends VP9 video to Dart, which records it to a WebM file.</p>
    <p><strong>Note:</strong> VP9 is NOT supported by Safari. Chrome only.</p>
    ...
    <script>
        ...
        async function runTest() {
            try {
                ...
                // Check VP9 support
                if (browser === 'safari') {
                    throw new Error('VP9 is not supported by Safari');
                }

                setStatus('Starting VP9 save_to_disk test for ' + browser);
                ...
                log('Received offer from Dart (VP9)');

                if (offer.sdp.includes('VP9')) {
                    log('VP9 codec found in offer', 'success');
                } else {
                    log('Warning: VP9 not found in offer', 'warn');
                }
                ...
                if (answer.sdp.includes('VP9')) {
                    log('VP9 codec in answer', 'success');
                }
                ...
                setStatus('Recording VP9 video (5 seconds)...');
                log('Sending VP9 video for recording...');
                ...
                if (result.success) {
                    log('TEST PASSED! VP9 file created: ' + result.fileSize + ' bytes', 'success');
                    setStatus('TEST PASSED - ' + result.fileSize + ' bytes recorded (VP9)');
                } else {
...
void main() async {
  final server = SaveToDiskVp9Server(recordingDurationSeconds: 5);
