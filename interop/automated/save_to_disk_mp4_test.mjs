/**
 * Automated Save to Disk MP4 Browser Test using Playwright
 *
 * Tests WebRTC video recording to fragmented MP4:
 * - Browser sends H.264 camera video to Dart server
 * - Dart depacketizes H.264 RTP and writes to fMP4 container
 * - Verifies MP4 file is created with valid size
 *
 * Usage:
 *   # First, start the Dart server in another terminal:
 *   dart run interop/automated/save_to_disk_mp4_server.dart
 *
 *   # Then run browser tests:
 *   node interop/automated/save_to_disk_mp4_test.mjs [chrome|firefox|webkit|all]
 */
...
async function runBrowserTest(browserType, browserName) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing Save to Disk MP4: ${browserName}`);
  ...
  try {
    ...
    console.log(`[${browserName}] Running save to disk MP4 test...`);
...
async function main() {
  ...
  try {
    ...
    console.error('\nError: Save to Disk MP4 server is not running!');
    console.error('Start it with: dart run interop/automated/save_to_disk_mp4_server.dart');
    process.exit(1);
  }
  ...
  console.log('\n' + '='.repeat(60));
  console.log('SAVE TO DISK MP4 TEST SUMMARY');
  console.log('='.repeat(60));
