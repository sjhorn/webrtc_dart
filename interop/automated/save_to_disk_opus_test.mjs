/**
 * Automated Save to Disk Opus Browser Test using Playwright
 *
 * Tests WebRTC Opus audio recording:
 * - Browser sends Opus microphone audio to Dart
 ...
 *   dart run interop/automated/save_to_disk_opus_server.dart
 *
 *   # Then run browser tests:
 *   node interop/automated/save_to_disk_opus_test.mjs [chrome|firefox|webkit|all]
 *
...
async function runBrowserTest(browserType, browserName) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`Testing Save to Disk Opus: ${browserName}`);
  ...
  try {
    ...
    // Grant microphone permissions (Chrome only - Firefox uses prefs above)
    ...
    console.log(`[${browserName}] Running Opus test (recording for 5 seconds)...`);
...
async function main() {
  ...
  try {
    ...
    console.error('\nError: Save to disk Opus server is not running!');
    console.error('Start it with: dart run interop/automated/save_to_disk_opus_server.dart');
    process.exit(1);
  }
  ...
  // Print summary
  console.log('\n' + '='.repeat(60));
  console.log('SAVE TO DISK OPUS TEST SUMMARY');
  console.log('='.repeat(60));
